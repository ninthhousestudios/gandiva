...
class ChartWheelWidget(QWidget):
    def __init__(self):
        ...
        self.cusp_positions   = []   # (roman_label, cx, cy, tooltip_text)
    ...
    def set_theme(self, name: str):
        ...
        clear_cache()                 # SVG color cache must be rebuilt
        self._apply_info_label_style()
        self.update()
    ...
    def _apply_info_label_style(self):
        ...
        self.info_label.setStyleSheet(f"""
            QLabel {{
                ...
                background-color: rgba({bg.red()},{bg.green()},{bg.blue()},210);
                padding: 6px 8px;
                border-radius: 4px;
                font-size: 13px;
            }}
    ...
    def update_from_chart(self, chart):
        ...
        # Center midpoint of house-1 sign at 9 o'clock (not the Ascendant degree)
    ...
    def _update_info_overlay(self, chart):
        ctx     = chart.context
        jd      = ctx.timeJD
        loc     = ctx.location
        utcoff  = jd.utcoffset
        utcsign = "+" if utcoff >= 0 else ""
        lat_dir = "N" if loc.lat >= 0 else "S"
        lon_dir = "E" if loc.long >= 0 else "W"
        lines = []
        if ctx.name:
            lines.append(f"<b>{ctx.name}</b>")
        lines.append(f"{jd.usrdate()}  {jd.usrtime()}")
        lines.append(f"UTC{utcsign}{utcoff:g}  —  {loc.placename()}")
        lines.append(f"{abs(loc.lat):.4f}° {lat_dir},  {abs(loc.long):.4f}° {lon_dir}")
    ...
    def _draw_sign_names(self, p, cx, cy, r, r_sign):
        ...
        if self.is_aditya:
            ...
            p.setPen(QPen(self.theme["sign_label"]))
    ...
    def _draw_house_ring(self, p, cx, cy, r_planet, r_house):
        ...
        p.setFont(font_h)
        p.setPen(QPen(self.theme["house_label"]))
        ...
        for i in range(1, 13):
            ...
            p.setPen(QPen(self.theme["cusp_label"], weight))
    ...
    def _draw_center_image(self, p, cx, cy, r_house):
        ...
        p.restore()
        p.setPen(QPen(self.theme["line"], 1.2))
        p.setBrush(Qt.BrushStyle.NoBrush)
        p.drawEllipse(QPointF(cx, cy), r_house, r_house)

    # ── spread algorithm ──────────────────────────────────────────────────────

    def _spread(self, items, min_sep=8.0):
        """Nudge items apart angularly so glyphs don't overlap.
        items: [(name, ecl, retro, info), ...]
        returns: [(name, ecl_spread, ecl_true, retro, info), ...]"""
        if not items:
            return []
        entries = [[name, math.degrees(self._ecl_to_angle(ecl)) % 360,
                    ecl, retro, info]
                   for name, ecl, retro, info in items]
        ...
        for _ in range(60):
            ...
            for i in range(len(entries)):
                j = (i + 1) % len(entries)
                a1, a2 = entries[i][1], entries[j][1]
                diff = (a2 - a1) % 360
                if diff < min_sep:
                    push = (min_sep - diff) / 2 + 0.1
                    entries[i][1] = (a1 - push) % 360
                    entries[j][1] = (a2 + push) % 360
                    moved = True
            if not moved:
                break

        return [(e[0], self._angle_to_ecl(e[1]), e[2], e[3], e[4])
                for e in entries]
    ...
    def mouseMoveEvent(self, event):
        ...
        if planet_hit:
            ...
            QToolTip.showText(gpos, planet_hit[1], self)
        elif cusp_tip:
            self.setCursor(Qt.CursorShape.PointingHandCursor)
            QToolTip.showText(gpos, cusp_tip, self)
        else:
